"""
LinkedIn Jobs Scraper India — No Login Required
-------------------------------------------------
Uses LinkedIn's public guest API endpoint to scrape job listings
in India without a session cookie, Selenium, or any account risk.
...
Run:  python linkedin_nologin_scraper_india.py
"""
...
SEEN_FILE = Path(__file__).parent / "json" / "linkedin_nologin_india_seen.json"

EMAIL_SENDER   = os.environ.get("EMAIL_SENDER",       "")
EMAIL_PASSWORD = os.environ.get("GMAIL_APP_PASSWORD",  "")
EMAIL_TO_LIST  = [e.strip() for e in os.environ.get("EMAIL_TO_INDIA", "").split(",") if e.strip()]
...
# Staffing/recruiting agency signals in the job description (no extra requests — description already fetched)
...
def send_email(new_jobs: list[dict]) -> None:
    ...
    if not EMAIL_TO_LIST:
        print("[!] EMAIL_TO_INDIA not set — skipping email.")
        return
    ...
    try:
        ...
        with smtplib.SMTP_SSL("smtp.gmail.com", 465) as srv:
            ...
            srv.sendmail(EMAIL_SENDER, EMAIL_TO_LIST, msg.as_string())
        print(f"[+] Email sent → {EMAIL_TO_LIST} ({len(new_jobs)} role(s))")
...
def main():
    print("=" * 55)
    print("LinkedIn Jobs Scraper India — No Login")
    print("=" * 55)
    ...
    if all_jobs:
        ...
        historical_max = max((int(jid) for jid in seen), default=0)
